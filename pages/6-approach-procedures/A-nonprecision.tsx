        ...
        knowledge(id) {
          switch (id) {
            case '1':
              return [
                <>
                  A <Term>nonprecision approach (NPA)</Term> is an instrument approach without
                  vertical guidance. Lateral guidance is provided by a navaid, conventionally a
                  ground-based radio system, but these days satellites are used as well. Common
                  examples of nonprecision approach procedures include VOR, localizer, LDA and LNAV
                  procedures, among others.
                </>,

                <>
                  Nonprecision approaches use <Term>minimum descent altitudes (MDAs)</Term> to
                  define how low a pilot may safely descend. The common technique for flying a
                  nonprecision approach is known as <Warning>"dive and drive"</Warning>, in which
                  the aircraft descends in a stepwise fashion from altitude to altitude (dive) and
                  then holds that altitude until passing the next fix on the approach which permits
                  a descent to a lower altitude (drive). The flight path looks a bit like a
                  staircase. When the aircraft arrives at the MDA, it is not permitted to descend
                  any lower until the runway environment is in sight (see{' '}
                  <Link.Task section={6} task="E" id="k1" />
                ...
                <>
                  <Danger>
                    In practice, it's usually clear well before the MAP if the missed approach will
                    be required.
                  </Danger>{' '}
                  One of the conditions for landing from an IAP is that the aircraft is continuously
                  in a position to land using normal maneuvers.{' '}
                  <Danger>
                    However, many MAPs are within a mile or so of the runway threshold at 1000 feet
                    AGL or higher; landing in such a situation would be highly abnormal and
                ...
                >
                  ...
                  <Term>visual descent point (VDP)</Term>. <AIM paragraph={[5, 4, 5, 'h']} />{' '}
                  defines the VDP like so:
                  <Quotation>
                    The visual descent point (VDP), identified by the symbol (V), is a defined point
                    on the final approach course of a nonprecision{' '}
                    <Bold>straight-in approach procedure</Bold>
                    from which a stabilized visual descent from the MDA to the runway touchdown
                  ...
                  begin the descent. Check out this{' '}
                  <Link href={references.flight_insight_vdp}>FlightInsight video</Link> for a
                  thorough discussion.
                </Paragraph>,
                ...
                <>
                  <Tabs>
                    <Tab heading="LNAV and LP">
                      ...
                      <Paragraph>
                        Nonprecision RNAV approaches include <Term>LNAV (LATeral NAVigation)</Term>{' '}
                        and <Term>LP (Localizer Performance)</Term> procedures. procedures. Both
                        types of procedure utilize a{' '}
                        <Term>global navigation satellite system (GNSS)</Term> to provide lateral
                        guidance. The primary difference is the degree of precision:{' '}
                        <Info>LP approaches utilize the Wide Area Augmentation System (WAAS)</Info>{' '}
                        while <Info>LNAV approaches only utilize GPS</Info>. Per{' '}
                        <AIM paragraph={[5, 4, 5, 'm']} />:
                        <Quotation>
                          LP will be published in locations where vertically guided minima cannot be
                          provided due to terrain and obstacles and therefore, no LPV or LNAV/VNAV
                          minima will be published.
                        </Quotation>
                      </Paragraph>
                      Like localizer approaches, an LP approach comes with "angular guidance":{' '}
                      <Info>lateral sensitivity incleases as the approach continues.</Info> Note
                      that{' '}
                      <Warning>
                      ...
                      <Link.Reference reference="TSO-C146" text="TSO-C146b" /> it may require an
                      upgrade to fly to LP minima. There must be a statement in the POH or
